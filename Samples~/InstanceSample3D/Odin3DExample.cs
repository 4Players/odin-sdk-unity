    ...
    public class Odin3DExample : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            else
            {
                ...
                // this example creates and configures medias itself (e.g. spatial audio per peer),
                // so disable the convenience default media creation
                _Room.AutoCreateMedia = false;
                ...
                _Room.OnDecoderAdded.AddListener(Example_OnCreatedMediaObject);
                _Room.OnDecoderRemoved.AddListener(Example_OnDeleteMediaObject);
        ...
        private void SetupUnityMicrophoneInput()
        {
            ...
            if (!mic)
            {
                Debug.Log("Example failure with OdinMicrophoneReader component");
                return;
            }
        ...
        public void Example_OnRoomJoined(object sender, RoomJoinedEventArgs args)
        {
            ...
            {
                /*
                 * processing the audio pipeline can have many different effects and are called in order
                 * add either a buildin component or directly with the pipeline and/or build one
                 *
                 * an effect is PipelineEffect : ICustomPipelineEffect and can work with any IMedia
        ...
        {
            Debug.Log($"PeerJoined : {args.peer_id}");

            OdinRoom room = sender as OdinRoom;
            var peerContainer = Instantiate(PeersPrefab, new Vector3(0, 0.5f, 6), Quaternion.identity);
            room.AddPeerComponent(peerContainer, args.peer_id);

            PeersObjects.TryAdd(args.peer_id, peerContainer);

            //set dummy PeerCube label
            var data = CustomUserDataJsonFormat.FromUserData(room.GetBaseRoom<Room>().RemotePeers[args.peer_id]?.UserData);
            peerContainer.GetComponentInChildren<TextMesh>().text = data == null ?
                $"Peer {args.peer_id} ({args.peer_id})" :
                $"{data.name} (Peer {args.peer_id})";

            // the current protocol does not announce medias: with AutoCreateMedia disabled
            // we create the decoder for the new peer manually, which triggers OnDecoderAdded
            // (see Example_OnCreatedMediaObject for the OdinDecoder component setup)
            room.CreateMediaDecoder(args.peer_id);
        }
        ...
        /// <summary>
        /// redirect the event and let the <see cref="OdinPeer"/> handle the decoder
        /// if a peer starts a new decoder we can create and configure the <see cref="OdinDecoder"/> component
        /// </summary>
        /// <remarks>we use the helper function <see cref="OdinPeer.AddDecoderComponent"/> either direct or with default eventhandler but a <see cref="GameObject.AddComponent{T}()"/> with a <see cref="OdinDecoder"/> that needs to be manually configured works too</remarks>
        /// <param name="sender">room where the event is dispatched from</param>
        /// <param name="args">event args that contains native infos from the event</param>
        public void Example_OnCreatedMediaObject(object sender, DecoderAddedEventArgs args)
        {
            if (PeersObjects.TryGetValue(args.PeerId, out var peerContainer))
            {
                ...
                decoder.OnActiveStateChanged.AddListener(Example_MediaActivityChanged);
                // this example playfield pane is small for default audible changes
                decoder.RolloffMode = AudioRolloffMode.Linear; // (AudioSource.RolloffMode)
                decoder.MaxDistance = 10.0f; // (AudioSource.MaxDistance)
                // after we setup enable the component
                decoder.enabled = true; // or SetActive
        ...
        /// show
        /// </summary>
        /// <param name="sender"><see cref="OdinDecoder"/> component</param>
        ...
        private void Example_MediaActivityChanged(object sender, MediaActiveStateChangedEventArgs args)
        {
            ...
            OdinDecoder decoder = sender as OdinDecoder;
            Material cubeMaterial = decoder.GetComponentInParent<Renderer>().material;
            if (args.Active)
            {
        ...
        /// <summary>
        /// we only update the label - redirect the event and let the <see cref="OdinPeer"/> handle the decoder
        /// if a still existing peer removes his decoder we remove it as well
        /// <code>
        /// // simple example
        /// OdinDecoder decoderComponent = peerContainer.GetComponent{OdinDecoder}();
        /// Destroy(decoderComponent);
        /// </code>
        /// </summary>
        /// <remarks>we use the helper function <see cref="OdinPeer.RemoveDecoderComponent"/> either direct or with default eventhandler but a <see cref="GameObject.GetComponent{T}()"/> with a <see cref="OdinDecoder"/> that needs to be destroyed works too</remarks>
        /// <param name="sender">room where the event is dispatched from</param>
        /// <param name="args">event args that contains native infos from the event</param>
        public void Example_OnDeleteMediaObject(object sender, DecoderRemovedEventArgs args)
        {
            if (PeersObjects.TryGetValue(args.PeerId, out var peerContainer))
            {
        ...
        // only for showcase
        private static void SetExamplePeerLabel(object sender, GameObject peerContainer, uint peerid, ulong mediaid = 0)
        {
            //set dummy PeerCube label
