    ...
    public class OdinLibraryHandle : OdinHandle
    {
            ...
            : base(platform, _SetNamedLib(platform, possiblePaths))
        {
            ...
            if (ret == OdinError.ODIN_ERROR_SUCCESS)
            {
                ...
                try
                {
                    NativeLibraryMethods.odin_debug_dump_state odds;
                    GetLibraryMethod("odin_debug_dump_state", out odds);
                    OdinLog.DumpState = odds;
                    NativeLibraryMethods.odin_debug_set_logging_hook odslh;
                    GetLibraryMethod("odin_debug_set_logging_hook", out odslh);
                    OdinLog.Hook = odslh;
                }
                catch { /* NOP */ }
                ;
...
#pragma warning disable CS0618 //UNITY Type or member is obsolete
                OdinLog.Throw(new OdinWrapperException($"Native library will be in an invalid state. (odin_initialize {ret})", new NotSupportedException(lastError)));
